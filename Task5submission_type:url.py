...
import requests
import json
import Task1submissiontype_url
top_movies=Task1submissiontype_url.scrap_top_movies()
def  get_movies_details(movies):
    c=0
    information_list=[]
    while c<len(movies):
        information_dict ={}
        ...
        print(c)
    with open("get_movies_details.json","w") as f:
        json.dump(information_list,f,indent=4)
get_movies_details(top_movies[:10])
